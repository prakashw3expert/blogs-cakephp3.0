...
$this->extend('/Common/Admin/categories');
...
if (!empty($categories)) {
    foreach ($categories->toArray() as $key => $listOne) {
        ...
        $row['id'] = $listOne['id'];
        $row['title'] = $listOne['title'];
        
        if (empty($parent_id)) {
            $row['child_count'] = $this->Html->link($listOne['child_count'], ['action' => 'index', $listOne['id']]);
        }
        $row['status'] = $this->Awesome->getLabedStatus($listOne['status']);
        $row['created'] = $listOne['created'];
...
$this->append('table_row', json_encode($rows));
