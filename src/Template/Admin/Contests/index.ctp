...
$tableHeaders[] = array(null => array('style' => 'width:30%'));

$tableHeaders[] = array($this->Paginator->sort(__('status')) => array('class' => 'text-center', 'style' => 'width:10%'));
$tableHeaders[] = array($this->Paginator->sort(__('enteries')) => array('class' => '', 'style' => 'width:10%'));
$tableHeaders[] = array($this->Paginator->sort(__('expiry'), 'End Date') => array('style' => 'width:10%'));
...
if ($contests->count() > 0) {
    ...
    foreach ($contests->toArray() as $key => $listOne) {
        $listOne = $listOne->toArray();

        $row = array();
        
        $row[] = $this->Awesome->image('Contests/image', $listOne['image'], ['class' => 'img-circle thumb-md clearfix']);
        $row[] = $label . $this->Html->link($listOne['title'],['controller' => 'contests','action' => 'view','slug' => $listOne['slug']]);

        $row[] = array($this->Awesome->getLabedStatus($listOne['status']), array('class' => 'text-center'));
        $row[] = $listOne['enteries'];
        $row[] = $listOne['expiry'];
...
echo $this->Form->create($contests, array(
    'novalidate' => true,
    'type' => 'get',
    'class' => 'form-inline',
));
