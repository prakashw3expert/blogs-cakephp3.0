...
$tableHeaders[] = array($this->Paginator->sort(__('created'), 'Date') => array('style' => 'width:30%'));
...
if ($events->count() > 0) {
    ...
    $row[] = array(__('You have not create your first event yet!', $modelClass), array('class' => 'text-center noresult', 'colspan' => 9));
    $rows[] = $row;
}
