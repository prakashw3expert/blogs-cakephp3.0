...
class AuthController extends AppController {
    ...
    public function initialize() {
        ...
        $this->Auth->allow(['facebook', 'google', 'twitter', 'resetPassword', 'google']);
    }
    ...
    public function facebook() {
        ...
        $params = [
            ...
            'client_secret' => '3a98f0b39144b88f7febbbbc5ffe1e9d'
        ];
        ...
        $status = false;
        if ($user = $user->first()) {
            if (!empty($this->loggedIn) && $this->loggedIn['facebook_id'] && $this->loggedIn['facebook_id'] != $user->facebook_id) {
        ...
        if ($status && !$this->loggedIn) {
            $user = $this->Users->find('all',['conditions' => ['Users.id' => $user->id],'contain' => ['Roles']])->first();
    ...
    public function google() {
        ...
        $params = [
            'code' => $this->request->data('code'),
            'client_id' => $this->request->data('clientId'),
            'client_secret' => 'PRJRPXIPHSd_SPTfOfvuXZay',
            'redirect_uri' => $this->request->data('redirectUri'),
...
// Step 3a. If user is already signed in then link accounts.
        ...
        $status = false;
        if ($user = $user->first()) {
            if (!empty($this->loggedIn) && $this->loggedIn['google_id'] != $user->google_id) {
...
// Step 5a. Link user accounts.
            ...
            $status = false;
            if ($user = $user->first()) {
                if ($this->loggedIn['twitter_id'] != $user->twitter_id) {
